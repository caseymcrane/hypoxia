...
class limbTorso:
	def __init__(self,hp,name,organs=[],armor=None):
		self.name = name
		self.max_hp = hp
		self.hp = hp
		self.armor = armor
	...
	for i in range(num_monsters):
		...
		if not is_blocked(x,y):
			if libtcod.random_get_int(0,0,100) < 80:
				...
				bruiser_torso= limbTorso(name='bruiser torso',hp=40)
				b_limbs=[bruiser_torso,bruiser_larm,bruiser_rarm,bruiser_lleg,bruiser_rleg,bruiser_head]
				bruiser_body = Body(limbs=b_limbs)
...
def player_move_or_attack(dx,dy):
	...
	if target is not None:
		for limb in limbs if limb.type == limbArm:
			limb.attack(
	else:
		player.move(dx,dy)
		fov_recompute = True
